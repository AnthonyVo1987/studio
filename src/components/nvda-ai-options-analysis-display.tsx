...
export function NvdaAiOptionsAnalysisDisplay() {
  const nvdaState = useNvdaAnalysis();

  // Derived state - data availability and loading state
  const isDataReady = nvdaState.hasAiOptionsAnalysis;
  const isLoading = nvdaState.isAiOptionsAnalysisLoading;
  ...
  const exportActions = useQuickExport(
    optionsAnalysisData,
    `${NVDA_TICKER}_ai_options_analysis`,
    `${NVDA_TICKER} AI Options Analysis`
  );
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-wrap gap-2">
            <Button 
              ...
              title={`Copy ${NVDA_TICKER} AI Options Analysis as JSON`}
            >
            ...
            <Button 
              ...
              disabled={!isDataReady}
              title={`Export ${NVDA_TICKER} AI Options Analysis as JSON`}
            >
          ...
          <div className="space-y-4">
            ...
            <div className="prose prose-sm max-w-none">
              <p className="text-sm text-muted-foreground">
                {NVDA_TICKER} AI options analysis completed successfully. Use the export buttons above to view the detailed insights.
              </p>
            </div>
          </div>
        ) : isLoading ? (
          <div className="text-center text-muted-foreground h-24 flex items-center justify-center">
            Generating {NVDA_TICKER} AI options analysis...
          </div>
        ) : (
          <div className="text-center text-muted-foreground h-24 flex items-center justify-center">
            No {NVDA_TICKER} AI options analysis available. Generate analysis first using the button above.
          </div>
        )}
      </CardContent>
    </Card>
