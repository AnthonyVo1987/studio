...
export function NvdaOptionsChainTable() {
  ...
  const handleExportOptionsJson = () => {
    if (!isDataReadyForExport || !parsedDataState) {
      toast({ variant: 'destructive', title: 'Data Not Ready', description: 'NVDA options chain data is not available for JSON export.' });
      return;
    }
  ...
  const handleCopyOptionsJson = async () => {
    if (!isDataReadyForExport || !parsedDataState) {
      toast({ variant: 'destructive', title: 'Data Not Ready', description: 'NVDA options chain data is not available for JSON copy.' });
      return;
    }
  ...
  const renderContent = () => {
    if (isLoadingState) {
      return <div className="p-4 text-center text-sm text-muted-foreground h-24 flex items-center justify-center">Waiting for NVDA options data...</div>;
    }
    if (isErrorState) {
    ...
    if (!parsedDataState || contractsToDisplay.length === 0) {
      return <div className="text-center h-24 p-4 text-muted-foreground">{errorOrSkippedMessageState || 'No NVDA option contracts found for this expiration and strike range.'}</div>;
    }
      ...
      <CardHeader>
        <div className="flex justify-between items-start">
            ...
            <div className="flex flex-wrap gap-2">
                <Button variant="outline" size="sm" onClick={handleCopyOptionsJson} disabled={!isDataReadyForExport} title="Copy NVDA Options Chain as JSON">
                    <Copy className="mr-2 h-4 w-4" /> Copy JSON
                </Button>
                <Button variant="outline" size="sm" onClick={handleExportOptionsJson} disabled={!isDataReadyForExport} title="Export NVDA Options Chain as JSON">
