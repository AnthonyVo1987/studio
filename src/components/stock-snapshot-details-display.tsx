...
export function StockSnapshotDetailsDisplay() {
  ...
  if (stockSnapshotJson && stockSnapshotJson !== '{}') {
    if (stockSnapshotJson.includes('"status": "initializing"') || stockSnapshotJson.includes('"status": "pending"') || stockSnapshotJson.includes('"status": "full_analysis_pending..."')) {
      ...
      try {
        ...
        console.debug("[StockSnapshotDetailsDisplay] Successfully parsed stockSnapshotJson. Ticker:", data?.ticker);
  ...
  console.debug(`[StockSnapshotDetailsDisplay] Render state: isLoading=${isLoading}, isError=${isError}, details.length=${details.length}, parsedSnapshotData exists=${!!parsedSnapshotData}`);
