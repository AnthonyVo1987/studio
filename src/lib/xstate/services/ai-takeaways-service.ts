/**
 * @fileOverview AI Takeaways Service - XState-compatible service wrapper
 * 
 * Wraps existing StockSage AI technical analysis functionality into an XState-compatible
...
function buildAIContextFromStockData(stockDataResult?: any): ContextData {
  ...
  return {
    stockSnapshotJson: data.stockSnapshotJson || '',
    marketStatusJson: data.marketStatusJson || '',
    technicalIndicatorsJson: data.technicalIndicatorsJson || '',
    optionsChainJson: data.optionsChainJson || '',
  };
}

/**
 * AI Takeaways Service Implementation
 * 
 * Wraps the existing Google Genkit AI analysis functionality
 * with XState-compatible Promise-based interface, timeout protection,
 * retry logic, and comprehensive error handling.
 */
export const createAITakeawaysService: () => AITakeawaysService = () => {
  return async (options: MacroServiceOptions & { stockDataResult?: any }): Promise<MacroServiceResult<AITakeawaysResult>> => {
    const {
      ticker,
      executionId,
      timeout = 45000,
    ...
    const logger = createTickerLogger(ticker, 'AITakeawaysService', executionId);
    const serviceContext = {
      serviceName: 'AITakeawaysService',
      stepId: 3,
      ticker,
      executionId,
      attempt: 0,
    ...
    if (enableDebug) {
      logger.stateValidation('ServiceStart', 'AI takeaways service starting', {
        ticker,
        executionId,
        timeout,
        maxRetries,
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      ...
      try {
        ...
        // Build context data from stock data result
        const contextData = buildAIContextFromStockData(stockDataResult);

        // Prepare AI chat input payload
        const chatInput = {
          ticker,
          promptName: 'stock-trader-takeaways',
          userInput: `Generate comprehensive technical analysis takeaways for ${ticker}`,
          webSearchEnabled: false,
          ...contextData,
        };

        // Execute the AI analysis operation with timeout protection
        ...
        if (!aiResponse || aiResponse.status !== 'success') {
          const errorMessage = aiResponse?.error || aiResponse?.message || 'AI analysis failed';
          throw createServiceError(
            new Error(`AI takeaways generation failed: ${errorMessage}`),
            'api',
            serviceContext
          );
        }
        ...
        // Parse the response to estimate token count (rough estimation)
        ...
        try {
          ...
          // Token estimation failed, continue without it
        }

        const result: AITakeawaysResult = {
          aiKeyTakeawaysJson: aiResponse.data.responseJson,
          promptUsed: 'stock-trader-takeaways',
          processingTime,
          tokenCount,
        ...
        if (enableDebug) {
          logger.stateValidation('ServiceSuccess', 'AI takeaways completed successfully', {
            ticker,
            executionId,
 ...
 */
export default createAITakeawaysService;